...
for kk=[1]
    ...
    Deltas=[0.0];
    % Vs=[1.0];%dV=Vs(2)-Vs(1);
%     Vs=[1.7:0.005:1.9];dV=Vs(2)-Vs(1);
    Vs=[2.95:0.001:3.05];dV=Vs(2)-Vs(1);
    % 2025-3-25 23:14:43
    narray=[14:20]; %987
...
%                     eigval=eig(H);
%                     % 提取特征值的实部
%                     eigval_real = real(diag(eigval));
%                     [~, sorted_indices] = sort(eigval_real);
%                     eigval_sorted = eigval(sorted_indices);
% 
%                     
%                     entangle(iV,jj)=eigval_sorted(2)-eigval_sorted(1);
...
% 按照实部排序特征值和特征向量
eigval_sorted = eigval(sorted_indices, sorted_indices);
eigvec_sorted = eigvec(:, sorted_indices);
%  entangle(iV,jj)=eigval_sorted(2,2)-eigval_sorted(1, 1);
%                     psi=eigvec(:,1);
                    psi=eigvec_sorted(:,1);
%                     ipr_jj(jj)=sum(abs(psi).^4); % .* log(abs(psi).^2)  -(1-abs(psi).^2) .* log(1-abs(psi).^2) );
                    entangle_jj_jj=sum(-(abs(psi).^2) .* log(abs(psi).^2)  -(1-abs(psi).^2) .* log(1-abs(psi).^2) );
 entangle(iV,jj)= entangle_jj_jj; %eigval_sorted(2,2)-eigval_sorted(1, 1);
% % 显示排序后的特征值
% disp('排序后的特征值（实部从小到大）：');
% disp(diag(eigval_sorted));
            ...
            for iV=1:length(Vs)
                V=Vs(iV);
                data=entangle(iV,:); % =fs(iV,jj);
                entangle_ave(iV,iN) = mean(data);  %^(1/length(data));
            ...
            ylabel('entangle');
            set(gca,'FontSize',15);
            title([ ' N=' num2str(N)  ' V=' num2str(V) ' mu=' num2str(mu) ' phi=' num2str(phi)])
...
%             saveas(gcf, [currentFileName  '_fs_N_' num2str(N) ' .png']);
            saveas(gcf, [currentFileName  '_entangle.png']);
    % 将数组保存为文本格式的 .dat 文件（空格分隔）
% save([currentFileName  '_fs_N_' num2str(N) '.dat'], 'tfs', '-ascii', '-double');
save([currentFileName  '_entangle_ave.dat'], 'entangle_ave', '-ascii', '-double');
        end
        % %     hold on
...
%     xline(x_val, '--r', 'LineWidth', 1.5);
    ...
    for x=[]
        ...
        % %  [X,Y]=meshgrid(aarray,harray);yarray=real(entangle);
